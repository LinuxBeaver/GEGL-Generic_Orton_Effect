...
property_double (radius2, _("Radius"), 2.0)
...
#define GEGL_OP_META
#define GEGL_OP_NAME     generic_orton
#define GEGL_OP_C_SOURCE generic-orton.c
...
static void attach (GeglOperation *operation)
{
  ...
  GeglNode *input, *output, *hl, *blur1, *blur2;

  input    = gegl_node_get_input_proxy (gegl, "input");
  output   = gegl_node_get_output_proxy (gegl, "output");

  hl = gegl_node_new_child (gegl,
  ...
  gegl_node_link_many (input, blur1, hl, output, NULL);
  gegl_node_link (input, blur2);

  gegl_node_connect_from (hl, "aux", blur2, "output");
...
gegl_op_class_init (GeglOpClass *klass)
{
  ...
  gegl_operation_class_set_keys (operation_class,
    "name",        "gegl:generic-orton",
    "title",       _("Generic Orton"),
    "categories",  "Orton Effect",
